    ...
    if shutil.which("dot") is not None:
        ...
        with open(filename + ".png", "w") as f:
            if num_genes > 100:  # Large graphs are generated faster with sfdp
                subprocess.run(
                    ["sfdp", "-x", "-Goverlap=scale", "-T", "png", dot_file], stdout=f
                )
            else:
                subprocess.run(["dot", "-T", "png", dot_file], stdout=f)


def generate_ESM_normalization_files(
    num_genes, nv_file, varf_file, tt_file, signal_file, signal_len, state_file
):
...
def generate_input_signal(signal_len, signal_file):
    ...
    arr[
        0
    ] = 0  # Initial molecule level is 0, otherwise grid phi initialization is non-trivial
    with open(signal_file, "w") as f:
        f.write(" ".join([str(x) for x in arr]))


def generate_gene_initial_states(
    num_genes, num_cells, num_ESMs, input_signal_file, state_file
):
    """
    ...
    with open(state_file, "w") as f:
        for _ in range(num_cells):
            ...
            for i in range(num_ESMs):
                state[
                    i + 1
                ] = 0  # Initial molecule levels are 0, otherwise grid phi initialization is non-trivial
            f.write(" ".join([str(x) for x in state]))
            f.write("\n")
