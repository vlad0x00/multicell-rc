...
- `run_args` runs simulations by using arguments from `args.csv` file. This file is normally generated by `generate_args`, but can be written in any other way to specify an array of arguments for a given analysis. The results are stored in `results.csv`.